...
public class WidgetHelper {
    ...
    public static void requireLibrary(String libraryName, VolantisProtocol protocol)
        ...
        try {
            ...
            if (protocol.getProtocolConfiguration()
                    .isFrameworkClientInstallerSupported()) {
                // If instaler is supported, we assume local versions available
                // TODO: to be replaced with URL-rewriter based solution
                // as part of Req.Client.Packager implementation
                int index = 0;
                if((index = libraryName.indexOf(".mscr")) != -1){
                    libraryName = libraryName.substring(0,index)+".js";                    
                }
                String url = LOCAL_JS_FILES_URL + libraryName; 
                scriptAssetReference =  createWrapper(
                        (DefaultComponentScriptAssetReference)scriptAssetReference,
                        url,protocol.getMarinerPageContext());
            } 
