...
public abstract class TransTable extends TransElement
        implements OptimizationConstants {
    ...
    public void setHeader(Element header) {
        ...
        if (cells.getWidth() != 0) {
            logger.warn("header-set-after-row", new Object[]{new Integer(cells.getWidth())});
        }
    }
     ...
     */
    public TransCell addCell(Element tr, Element td, int row, int col)
     ...
     */
    public TransCell getCell(int row, int col) {
    ...
    public void preprocess() {
        ...
        for (ListIterator i = depthFirst.listIterator(depthFirst.size());
             i.hasPrevious();) {
            TransCell cell = (TransCell) i.previous();
            cell.preprocess();
        }
     ...
     */
    protected void refactor(DOMProtocol protocol,
                            TransTable outerTable,
                            Element outerRow,
                            Element outerCell,
                            int requiredRows,
                            int requiredCols) {
        // This table's cells need to be "promoted" into the outerTable's given
        ...
        Element currentRow = outerRow;
        Element currentCell = outerCell;
        Element nestedRow;
        Element nestedCell;
        Element nextRow;
        Element nextCell;
        ...
        for (int section = 0; section < 3; section++) {
            // Determine the sections to be visited. The order in which
            // they should be visited is header then body then footer, such
            ...
            default:
                element = null;
                logger.error("invalid-iteration");
                break;
            }

            // Only examine non-null sections
            if (element != null) {
                for (nestedRow = (Element)element.getHead();
                     nestedRow != null;
                     nestedRow = nextRow) {
                    nextRow = (Element)nestedRow.getNext();

                    // Only row elements are of interest to this code
                    if (helper.isRow(nestedRow)) {
                        if (newRowNum != 0) {
                            ...
                            if ((object == null) ||
                                ...
                                newRows = new ArrayList(requiredRows);
                        ...
                        for (nestedCell = (Element)nestedRow.getHead();
                             nestedCell != null;
                             nestedCell = nextCell) {
     ...
     */
    protected void preserveStyle(Element cell,
                                 Element outerRow,
     ...
     */
    private void preserveStyleAttributes(Element cell,
                                           Element ancestor,
                                           String[] attributes) {
        if (attributes != null) {
            String cellAttr;
            ...
            for (int i = 0; i < attributes.length; i++) {
                cellAttr = cell.getAttributeValue(attributes[i]);

                if (cellAttr == null) {
                    ancestorAttr = ancestor.getAttributeValue(attributes[i]);

                    if (ancestorAttr != null) {
                        cell.setAttribute(attributes[i],
                                          ancestorAttr);
                    }
                }
            }
    ...
    private void promoteCell(Element cell,
        ...
        preserveStyle(cell, outerRow, originalCell,
   ...
   /**
    ...
    private void determineOptimizationLevel() {
        ...
        if (!protocol.supportsNestedTables()) {
            ...
            if (table != null) {
                ...
                if (optimization != null) {
                    if (OPTIMIZE_NEVER.equals(optimization)) {
                        optimizationLevel = OptimizationLevel.NEVER;
                    } else if (OPTIMIZE_ALWAYS.equals(optimization)) {
                        optimizationLevel = OptimizationLevel.ALWAYS;
                    } else if (OPTIMIZE_LITTLE_IMPACT.equals(optimization)) {
        ...
        if (table != null) {
            table.removeAttribute(OPTIMIZATION_ATTRIBUTE);
        }
     ...
     */
    public boolean canOptimize(TransCell containingCell) {
        boolean result = true;
     ...
     */
    private final LinkedList depthFirst = new LinkedList();
}
