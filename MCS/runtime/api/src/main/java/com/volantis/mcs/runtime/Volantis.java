...
public class Volantis {
    ...
    public void flushAllPipelineCaches() {
        // flush named caches
        Cache cache = retrievePipelineCache();
        cache.getRootGroup().flush(null);

        // flush urid:fetch and webd:get cache
        ...
        cache = urlContentCacheConfig.getCache();
        if (cache != null) {
            cache.getRootGroup().flush(null);
        }
    }
    ...
    public boolean flushPipelineCache(final String name) {
        ...
        // findGroup doesn't throw an IllegalStateException if it fails to find
        // something.
        final Group group = cache.getRootGroup().findGroup(name);
     ...
     * @return the Cache.
     */
    private Cache retrievePipelineCache() {
