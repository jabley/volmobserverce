...
public class XFormEmulator {
    ...
    public Document transform(DOMProtocol protocol, Document document) {
        ...
        document.forEachChild(implicitElementsExtractorNodeIteratee);
        
        // After all the implicit elements have been extracted,
        // process the document
        document.forEachChild(transformingNodeIteratee);
            ...
            if (formParent == null) {
                ...
                if (fd != null) {
                    ...
                    if (xfformElement != null) {
                        ...
                        // Links should be inserted as children of the xfform
                        // element if it is not null.
                        formParent = xfformElement;
                    }

                    // Check if the form parent can contain form links, and if not
                    // add in the necessary elements (under the xfform element) to
                    // enable it.
    ...
    protected boolean isValidFormParent(Element candidateParent) {
        ...
        if (candidateParent != null) {
            ...
            LOGGER.error(exceptionLocalizer.format("xforms-processing-error"));
        }        
        return result;
    }
