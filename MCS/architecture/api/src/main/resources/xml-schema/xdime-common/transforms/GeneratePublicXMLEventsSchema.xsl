...
<xsl:stylesheet xmlns:common="http://exslt.org/common" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0" exclude-result-prefixes="common xsi">
 ...
 ! ============================================================================
 ! This document defines the subset of XML Events that is supported within the
 ! XDIME 2 Language.
 ! ======================================================================== </xsl:comment>
    </xsl:template>

    <!-- Include all the XML Events modules when included from here. -->

    <xsl:template match="xs:include[contains(@schemaLocation, 'modules/xml-events-module.xsd')]">
        <xsl:call-template name="IncludeSchema">
            <xsl:with-param name="heading">XML Events Module</xsl:with-param>
        </xsl:call-template>
    </xsl:template>

    <!-- Purposely ignore all the XML Events modules when included from other modules. -->
    <xsl:template match="xs:include[starts-with(@schemaLocation, 'xml-events-')]"/>
