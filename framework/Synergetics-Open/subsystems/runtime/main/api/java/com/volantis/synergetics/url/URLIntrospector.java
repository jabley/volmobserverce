...
public class URLIntrospector implements Cloneable {
    ...
    public void addParameterValue(String name, String value) {
        ...
        // Allow null value to reconstruct URL to maximal compatible degree
        values.add(value);
    }
    ...
    private void parseQueryString(String query) {
        ...
        while (queryTokenizer.hasMoreTokens()) {
            String nameValue = queryTokenizer.nextToken();
            int separatorIndex = nameValue.indexOf('=');
            if (separatorIndex > 0) {
                // Correct name value pair (that includes empty value).
                addParameterValue(
                        decode(nameValue.substring(0, separatorIndex)),
                        decode(nameValue.substring(separatorIndex+1)));
            } else if (separatorIndex < 0) {
                // Name only; do store this with null value!
                addParameterValue(decode(nameValue), null);
            } else {
                // Parameter name not given - ignore it entirely.
            }
        }
    }
     ...
     * <p> This is needed because in JDK 1.2 URLDecoder.decode declares that it
     * throws Exception. </p>
     *
    ...
    private void reconstructQueryString(StringBuffer sb) {
        ...
        for (Iterator i = queryParameters.entrySet().iterator();
            ...
            for (Iterator j = values.iterator(); j.hasNext();) {
                ...
                sb.append(seperator).append(URLEncoder.encode(name));
                if (value != null) {
                    sb.append('=')
