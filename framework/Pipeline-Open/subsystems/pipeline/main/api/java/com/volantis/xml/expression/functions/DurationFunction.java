...
/**
 * See <a href="http://www.w3.org/TR/xquery-operators/#func-empty">fn:duration</a>.
 */ 
public class DurationFunction implements Function {
    ...
    private static final ExceptionLocalizer EXCEPTION_LOCALIZER =
        LocalizationFactory.createExceptionLocalizer(DurationFunction.class);
    ...
    public Value invoke(ExpressionContext expressionContext, Value[] values)
        ...
        if(values[0].getSequence() != Sequence.EMPTY) {
            // The date/time value must match the pattern defined in the
            // {@link #PATTERN} regular expression in order to be valid
            String duration = values[0].stringValue().asJavaString();
            result = expressionContext.getFactory().createDurationValue(duration);
        }
        return result;
    }
}
