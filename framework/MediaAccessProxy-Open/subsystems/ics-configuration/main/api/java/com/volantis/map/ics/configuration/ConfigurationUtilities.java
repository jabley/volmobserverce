...
public class ConfigurationUtilities {
    ...
    /**
     * Serialize a number representing TCP/IP port
     */
    public static String serializePort(int port) {
        return (port > 0) ?  Integer.valueOf(port).toString() : null;
    }

    /**
     * Deserialize a number representing TCP/IP port
     */
    public static int deserializePort(String string) {
        int result = 0;
        if (null != string) {
            string = string.trim();
            if (string.length() > 0) {
    ...
    public static int[] listToIntArray(String list, int defValues[])
        ...
        for (int i = 0; i < result.length; i++) {
            result[i] = (i < argList.size() && argList.get(i) != null) ?
                    (Integer) argList.get(i) : defValues[i];
        }
        return result;
    }
}
