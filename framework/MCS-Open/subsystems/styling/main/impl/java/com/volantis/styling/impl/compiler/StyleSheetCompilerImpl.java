...
import com.volantis.styling.impl.engine.selectionstates.SelectionStateFactory;
...
import com.volantis.styling.impl.sheet.StateChangeStylesDelta;
...
public abstract class StyleSheetCompilerImpl
    ...
    /**
     * Factory for {@link SelectionState}s.
     */
    private static final SelectionStateFactory SELECTION_STATE_FACTORY =
            SelectionStateFactory.getInstance();
...
//        List importantStylers = new ArrayList();
                ...
                if (values != null && values.length > 0) {
                    ...
                    if (matcher instanceof CompositeMatcher) {
                        Styler[] splitStylers = splitCompositeMatcher(source,
                                ...
                                (CompositeMatcher) matcher, delta, styleIndex);
                        for (int j = splitStylers.length - 1; j >= 0; j--) {
    ...
    /**
     * Create an array of {@link Styler}s corresponding to the individual parts
     * of the composite matcher.
     *
     * @param source
     * @param priority
     * @param specificity The specificity of the composite matcher
     * @param matcher The composite matcher to split
     * @param delta The style delta to apply on the final match of the composite
     * @return An array of simple stylers that correspond to the individual
     *         parts of the composite matchers. May contain nulls if the
     *         relevant styler exists already
     */
    ...
    /**
     * Generates a composite state for the specified operator.
     *
     * @param operator The operator for the composite selector
     * @return A composite state object for the specified operator
     */
    private SelectionState createCompositeState(Operator operator) {
        return SELECTION_STATE_FACTORY.createSelectionState(operator);
    }
