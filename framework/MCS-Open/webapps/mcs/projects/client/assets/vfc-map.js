...
Widget.GoogleMap = Widget.define(Widget.Appearable, Widget.Disappearable, Widget.ActionOwner, Widget.PropertyOwner, Widget.InputContainer,Widget.CallbackMixin , {
  ...
  _getImgSrc : function(src){
    ...
    if(src['t'] != undefined){
      ...
      //TODO - currently photo images are requested by the same way as map images
      // but it needs some changes in map service in order to work as in the past photo and map modes used 
      // different parameters and host - look at SwitchModeOperation.java
      // currently switch mode is probably not needed as only difference in in v parameter value
      imgSrc = this.icsSatHostPrefix+"&x="+imgX+"&y="+imgY+"&zoom="+this.currZoom+"&"+this.icsSatHostSuffix;
