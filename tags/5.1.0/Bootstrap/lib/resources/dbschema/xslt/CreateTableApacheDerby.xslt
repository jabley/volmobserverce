...
ALTER TABLE </xsl:text>
            ...
            <xsl:choose>
                <xsl:when test="string-length(sql:name) &gt; 15">
                    <xsl:value-of select="substring(sql:name,1,12)"/><xsl:value-of select="position()"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="sql:name"/>
                </xsl:otherwise>
...
<!--
 ...
 27-Jul-05	506/9	adrianj	VBM:2005071504 Moving database DDL script generation into Synergetics
