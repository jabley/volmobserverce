...
public class MatcherBuilderImpl
    ...
    public void visit(CombinedSelector selector) {
        ...
        CompositeStateFactory combinatorStateFactory = null;
        if (combinator == CombinatorEnum.DESCENDANT) {
            combinatorStateFactory = factory.createDescendantStateFactory();
        } else if (combinator == CombinatorEnum.CHILD) {
            combinatorStateFactory = factory.createChildStateFactory();
        } else if (combinator == CombinatorEnum.INDIRECT_ADJACENT) {
            combinatorStateFactory =
                    factory.createPrecedingSiblingStateFactory();
        } else if (combinator == CombinatorEnum.DIRECT_ADJACENT) {
            combinatorStateFactory =
                    factory.createImmediatelyPrecedingSiblingStateFactory();
        } else {
        ...
        constructedMatcher = factory.createCompositeMatcher(
                context, subject, combinatorStateFactory, builderContext);
    }
