    ...
    public static Value compute(Sequence l, Sequence r, BinaryOperator operator)
        ...
        if (l.getLength() == 0 || r.getLength() == 0) {
            ...
            if ((left instanceof NumericValue) &&
                ...
                // we raise that dynamic error
                throw new ExpressionException("Cannot compute the value of the operator " +
                                              "as we can only operate on numeric types: " +
                                              "left is of type " +
                                              left.getClass().getName() +
                                              " value \"" +
                                              left.stringValue().asJavaString() +
                                              "\" and right is of type " +
                                              right.getClass().getName() +
                                              " value \"" +
                                              right.stringValue().asJavaString() + '\"');
            }
