...
public class AssetLoader
        implements BuilderBatchOperation {

    /**
     * Used for logging
     */
    private static final LogDispatcher logger =
                LocalizationFactory.createLogger(AssetLoader.class);


    /**
     * Factory to create image assets from image files.
     */
    private final FileImageAssetFactory assetFactory =
            new FileImageAssetFactory();
    ...
    public AssetLoader(
            ...
            DeviceRepository deviceRepository, Project project) {

        if (sourceDirectory == null) {
            throw new IllegalArgumentException("sourceDirectory cannot be " +
                    "null");
        }
        if (deviceRepository == null) {
            throw new IllegalArgumentException("deviceRepository cannot be " +
                    "null");
        }
        if (project == null) {
    ...
    public void setAssetGroupName(String assetGroupName) {
        ...
        if (!(assetGroupName.startsWith(prefix) &&
                assetGroupName.endsWith(suffix))) {
            throw new IllegalArgumentException("asset group name must start " +
    ...
    public void setWidthHint(int widthHint) {
        ...
        if (widthHint < 0 || widthHint > 100) {
            throw new IllegalArgumentException("widthhint must be >= 0 and " +
                    "<= 100");
        }
        ...
        sourceDirEx.forEachDirectory(new FileExOperation() {
            public void perform(File policyDir) {
                ...
                try {
                    ...
                    logger.error("processing-component",
                            policyDir.getName(), e);
                }
            }
        });
            ...
            typeDirEx.forSingleFile(new FileExOperation() {
                public void perform(File assetFile) {
                    ...
                    try {
                        ...
                        logger.error(
                                "convertable-asset-file-added-for-component-error",
                                new Object[] {assetFile,  policyBuilder},
                                e);
                    }
                }
     ...
     * @param componentDir the directory that is the root of the component
     */
    private void createDeviceImageAssets(
            final VariablePolicyBuilder policyBuilder,
            File componentDir) {

        File typeDir = new File(componentDir, "device");
        if (typeDir.exists() && typeDir.isDirectory()) {
            logger.info("device-asset-for-component-processed",
                        policyBuilder);
            FileEx typeDirEx = new FileEx(typeDir);
                ...
                public void perform(File deviceDir) {
                    ...
                    try {
                        ...
                        if (device != null) {
                            ...
                            logger.warn(
                                    "device-directory-invalid-for-component",
                                    new Object[] {deviceName,
                                                  policyBuilder});
                        }
                    } catch (Exception e) {
                        logger.error("processing-device-for-component-error",
                                     new Object[] {deviceName,
                                                   policyBuilder},
                                     e);
                    }
     ...
     */
    private void createDeviceImageAsset(final VariablePolicyBuilder policyBuilder,
            File deviceDir) {

        final String deviceName = deviceDir.getName();
        logger.info("processing-device-asset",
                    new Object[] {deviceName,  policyBuilder});
        ...
        deviceDirEx.forSingleFile(new FileExOperation() {
            public void perform(File assetFile) {
                ...
                try {
                    ...
                    logger.error("device-asset-file-adding-error",
                                 new Object[] {assetFile,
                                          deviceName,
                                          policyBuilder},
                                 e);
     ...
     * @param componentDir the directory that is the root of the component
     */
    private void createGenericAsset(final VariablePolicyBuilder policyBuilder,
            File componentDir) {
        ...
        for (int i = 0; i < variants.size(); i++) {
            ...
            if (existing.getSelectionBuilder().equals(selectionBuilder)) {
                ...
                if (replace) {
                    ...
                    // ... else all we can do is log a warning.
                    logger.info("found-existing-ignoring-new",
                                new Object[] {existing, variantBuilder});
                    return;
                }
            }
        }
    ...
    private static class FileEx {
        ...
        void forEachFile(FileExOperation operation) {
            ...
            if (files == null) {
                throw new IllegalStateException("Cannot iterate over " +
                        "children of file");
            }
...
/*
 ...
 27-May-05	8542/6	pcameron	VBM:2005051701 Folder option's value defaults to / and must begin with /

 29-Apr-05	7361/1	geoff	VBM:2005031007 Provide device repository merge report
 ...
 24-Feb-05	7129/2	philws	VBM:2005011701 Ensure logger info, warn and error calls are localizable
 ...
 18-Aug-04	5177/4	geoff	VBM:2004081014 Provide a bulk image loading CLI (rework issues)

 16-Aug-04	5177/2	geoff	VBM:2004081014 Provide a bulk image loading CLI
