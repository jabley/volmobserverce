 ...
 */
public class AcceptParserTestCase extends TestCase {
     ...
     * the acceptable contents of the accept charset parser against an array of
     * charsets.
     */
     ...
     *
     * @todo reinstate this test case when charset parser has been ported here
     */
/*
...
*/
    ...
    private void doDefaultParserTest(String[] headers, String[] expected) {
        doTest(new AcceptParser(), headers, expected);
    }
    
    /**
     * Tests that sorting the headers retrieves the expected charsets
     * 
     * @param headers
     *            the http Accept-* header
     * @param expected
     *            an array of the expected entires in order
     */
    private void doSortParserTest(String[] headers, String[] expected) {
        doTest(new AcceptParser(true), headers, expected);
    }
    
    /**
     * Test that a default AcceptParser works with Accept-Language headers
     */
    public void testSortSingleToken() {
        doSortParserTest(new String[] { "en" }, new String[] { "en" });
    }
    
    /**
     * Test that a default AcceptParser works with multiple quality values with
     * qvalues < 1.
     */
    public void testSortMultipleCharsetQvalue() {
        doSortParserTest(new String[] { "A;q=0.5", "B;q=0.600", "C;q=0.2",
                "D;q=0.5" }, new String[] { "B", "A", "D", "C" });
    }
    
    
    /**
     * Test that a default AcceptParser works with multiple entries which are
     * defined without quality values
     */
    public void testSortEntriesWithoutQvalues() {
        doSortParserTest(new String[] { "C", "B", "A" }, new String[] { "A",
                "B", "C", });
    }
    
    /**
     * Test that a default AcceptParser works If the sorting is valid when
     * headers have equal values of quality
     */
    public void testSortEntriesWithEqualQvalues() {
        doSortParserTest(new String[] { "C;q=0.3", "B;q=0.3", "A;q=0.3" },
                new String[] { "A", "B", "C", });
    }
    
    /**
     * Test that a default AcceptParser correctly sorts groupings of equal
     * quality
     * 
     */
    public void testSortEntryGroupsWithEqualQvalues() {
        doSortParserTest(
                new String[]{"C;q=0.3", "B;q=0.3", "A;q=0.3",
                             "D;q=0.5", "E;q=0.5",
                             "G", "F"},
                new String[]{"F", "G", "D", "E", "A", "B", "C"});
    }
     ...
     */
    private void doTest(AcceptParser parser,
                        String[] headers,
                        String[] expected) {
    ...
    /**
     * Test that a default AcceptParser works with Accept-Language headers and
     * specificially _does not_ add the ISO-8859-1 value
     */
    ...
    /**
     * If items have equal q= values then us the order in which they appear in
     * the header. It isn't mentioned in the rfc but it seems like it would be
     * expected.
     */
...
*/
    ...
    /**
     * Test ISO-8859-1 with qvalue 0 (not acceptable). ISO-8859-1 will
     * <b>not</b> be added when it is already present as an explicit
     * unacceptable value.
     */
