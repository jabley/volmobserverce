...
/**
 * This class is responsible for writing an image as an 16 level greyscale GIF.
 * <p> If a maximum image size is set in the <code>Parameters</code> then the image
 * may be written as a 4 level greyscale GIF or scaled to fit the maximum size
 * set depending on the parameters settings.
 */
public class GIFGreyscale16Writer extends DefaultGIFWriter {

    /**
     * Used for logging.
     */
    private static final LogDispatcher LOGGER =
        LocalizationFactory.createLogger(GIFGreyscale16Writer.class);
    ...
    private static final ExceptionLocalizer EXCEPTION_LOCALIZER =
        LocalizationFactory.createExceptionLocalizer(
            GIFGreyscale16Writer.class);

    /**
     * Attempts to output an image in 16 level grayscale, however if a maximum
     * image size is set then the image is either reduced to 4 levels or scaled
     * to fit the maximum image size.  The option chosen depends on the
            ...
            if (params.containsName(ParameterNames.MAX_IMAGE_SIZE) &&
                ...
                if ((sizeOfConvertedImage > maxImageSize) &&
                    ...
                    os = ImageWriterFactory.getInstance().
                        getWriter(OutputImageRules.GREYSCALEGIF2, params).process(ops, params, os);
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("Changing the ImageWriter to 'gg2'");
                    }
                } else {
