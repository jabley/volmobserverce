...
/**
 * This class is responsible for writing an image as a 256 level greyscale PNG.
 * <p> If a maximum image size is set in the <code>DefaultConfiguration</code> then
 * the image may be written as a 16 level greyscale PNG or scaled to fit the
 * maximum size set depending on the configuration settings.
 */
public class PNGGreyscaleWriter extends DefaultPNGWriter {

    /**
     * Used for logging.
     */
    private static final LogDispatcher LOGGER =
        LocalizationFactory.createLogger(PNGGreyscaleWriter.class);

    /**
     * Used for localizing exceptions.
     */
    private static final ExceptionLocalizer EXCEPTION_LOCALIZER =
        LocalizationFactory.createExceptionLocalizer(PNGGreyscaleWriter.class);

    /**
     * Attempts to ouput an image in 256 level greyscale, however if a maximum
            ...
            if (params.containsName(ParameterNames.MAX_IMAGE_SIZE) &&
                ...
                if ((sizeOfConvertedImage > maxImageSize) &&
                    ...
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("Changing the ImageWriter to 'gp4'");
                    }
                } else {
