 ...
 * single value policy types in an unordered set.
 */
final class UnorderedSetComposition extends PolicyTypeComposition {
    /**
     * Construct a new UnorderedSetComposition.
     */
    UnorderedSetComposition() {
        super("unorderedSet");
    }
    ...
    void addTypeElementImpl(Element parent,
        ...
        Element unorderedSetElement =
                factory.element(DeviceRepositorySchemaConstants.
                POLICY_DEFINITION_UNORDEREDSET_ELEMENT_NAME,
                        parent.getNamespace());

        type.addPolicyTypeElement(unorderedSetElement, factory);

        parent.addContent(unorderedSetElement);
    }
    ...
    PolicyType getPolicyTypeImpl(Element type) {
        Element unorderedSet = (Element) type.getChildren().get(0);
        List children = unorderedSet.getChildren();
