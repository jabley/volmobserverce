...
public class WizardDefaultRenderer extends WidgetDefaultRenderer implements
    ...
    private static ScriptModule createAndRegisterModule() {
        ...
        ScriptModule module = new ScriptModule("/vfc-wizard.mscr", dependencies,
                16600, true);
    ...
    /**
     * Id input submit element.
     * It is needed for changing type attribute in case when wizard has validators
     */
    ...
    public void doRenderClose(VolantisProtocol protocol,
        ...
        // change input type for complete button in order to last step validator can works
        if (validatorsBuffer.length() > 0) {
            validatorsBuffer.append("Prototype.changeInputType($('" + submitId + "'), 'button');");
        }
