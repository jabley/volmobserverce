 ...
 * from 0 to 1.
 */
public class AlignedRequiredSlicesImpl
    ...
    private final int maxSubRowsPerSpatialCell;

    public AlignedRequiredSlicesImpl(
            boolean[][] requiredRows,
            boolean[][] requiredCols,
            int totalSubColumnsAcrossAllSpatialColumns,
            int maxSubRowsPerSpatialCell) {

        this.requiredRows = requiredRows;
        this.requiredColumns = requiredCols;
        this.totalSubColumnsAcrossAllSpatialColumns =
                totalSubColumnsAcrossAllSpatialColumns;
        this.maxSubRowsPerSpatialCell = maxSubRowsPerSpatialCell;
    }

    // Javadoc inherited.
    public boolean isSpatialRowRequired(int row) {
        return requiredRows[row] != null;
    }

    // Javadoc inherited.
    public boolean isSpatialColumnRequired(int column) {
        return requiredColumns[column] != null;
    }
