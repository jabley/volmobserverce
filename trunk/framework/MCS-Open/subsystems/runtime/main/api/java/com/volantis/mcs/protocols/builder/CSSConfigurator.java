...
import com.volantis.mcs.themes.StylePseudoSelectors;
import com.volantis.mcs.themes.StylePseudoSelectorIteratee;
...
public class CSSConfigurator implements ProtocolDeviceConfigurator {
    ...
    /**
     * Iterate over the style pseudo selectors (defined in
     * {@link StylePseudoSelectors}) and update the cssVersion with any values
     * specified for the individual selector by the device.
     *
     * @param device        which may specify different levels of support for
     *                      individual style pseudo selectors
     * @param cssVersion    to update with the device specific information
     */
    private void initialiseStylePseudoSelectors(
        ...
        StylePseudoSelectors.iterate(new StylePseudoSelectorIteratee() {
            public IterationAction iterate(String pseudoSelector) {
                ...
                if ("full".equals(value)) {
                    // Add the pseudoSelector definition to the css version.
                    cssVersion.addPseudoSelectorId(pseudoSelector);
                    if (logger.isDebugEnabled()) {
                        logger.debug("Adding device CSS pseudoSelector '" +
                                pseudoSelector + "'");
                    }
                } else if ("none".equals(value)) {
                    // Remove the pseudoSelector definiton from the css version.
                    cssVersion.removePseudoSelectorId(pseudoSelector);
                    if (logger.isDebugEnabled()) {
                        logger.debug("Removing device CSS pseudoSelector '" +
                                pseudoSelector + "'");
                    }
                } else if (value == null || "default".equals(value)) {
                    // Leave the original definition.
