...
public class MatcherContextImpl
    ...
    public MatcherContextImpl(ElementStack stack) {
        ...
        this.unregisterer = new StateUnregisterer() {
            public void unregisterState(StateMarker state, StylerContext context) {
                stateMarkers.remove(state.getState());
                if (context != null) {
                    context.getDepthChangeListeners().removeListener(state);
                }
            }
        };
    }
    ...
    private final Map stateMarkers = new HashMap();

    private final StateUnregisterer unregisterer;
