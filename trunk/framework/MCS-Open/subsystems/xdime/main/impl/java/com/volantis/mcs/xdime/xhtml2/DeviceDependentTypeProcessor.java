...
public class DeviceDependentTypeProcessor {
    ...
    private void doImageRequest(XDIMEContextInternal context, 
        ...
        try {
            ...
            // change url only if url's prefix in object src match media-access-proxy in
            // mcs-config.xml we know then that it is old ICS src format
            String newUrl = prepareUrl(pageContext, objectAttribute.getSrc());
            boolean isOldIcsUrl = false;
            if (! newUrl.equals(objectAttribute.getSrc())) {
                isOldIcsUrl = true;
            }

            newUrl = ContextInternals.constructImageURL(requestContext, newUrl);            

            URI finalURI = null;
            if (isOldIcsUrl) {
                // can't resolve against the page as sometimes we have to pass to MAP relative url
                finalURI = new URI(newUrl);
            } else {
                // try to resolve against the page. This does nothing if the url is absolute
                finalURI = new URI(pageContext.getAbsoluteURL(
                        new MarinerURL(newUrl), true).toExternalForm());
                // use resovled absolute uri from now on
                newUrl = finalURI.toString();
            }
    ...
    /**
     * In case of old ICS format url inside objectSrc absolute objectSrc will be changed into relative url.
     * MAP will itself add image host and port either form query string parameters (v.imgHost, v.imgPort) or
     * from MAP configuration file. Examples:
     * http://localhost:8080/map/ics/images/start/ubik_banner.jpg?v.imgHost=www.ubik.com => images/start/ubik_banner.jpg?v.imgHost=www.ubik.com
     * http://localhost:8080/map/ics/images/start/ubik_banner.jpg => /images/start/ubik_banner.jpg
     * @param pageContext MarinerPagecontext
     * @param objectSrc  src attribute value from object tag
     * @return new relative url or not modified objectSrc
     * @throws MalformedURLException
     */
    private String prepareUrl(MarinerPageContext pageContext, String objectSrc) throws MalformedURLException {

        String result = objectSrc;
        String mapUrl = pageContext.getVolantisBean().getMapUrlPrefix();

        String mapPrefix = mapUrl.substring(0,mapUrl.lastIndexOf('/'));
        if (objectSrc.startsWith(mapPrefix)) {
            // old ICS format
            int pathStartPosition = objectSrc.indexOf('/', mapPrefix.length() + 1);
            result = objectSrc.substring(pathStartPosition + 1, objectSrc.length());
        }
        return result;
    }
