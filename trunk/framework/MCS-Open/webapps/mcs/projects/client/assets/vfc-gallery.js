...
var Gallery = {
  ...
  createGallery: function(options) {
    ...
    var slideShow = $W(options.slideshow);
    if(slideShow != null) {
      slideShow.setPopupWidget(options.slideshowPopup);
    }
 ...
 * A slideshow widget.
 */
Gallery.Slideshow = Widget.define(Widget.Observer,
{
  items: null,
  display: null,
  ...
  /**
   * Set popupId needed for dismissed observation event raised from popup
   */
  setPopupWidget: function(popupId) {
    this.popupWidget = $W(popupId);
    if (this.popupWidget) {
