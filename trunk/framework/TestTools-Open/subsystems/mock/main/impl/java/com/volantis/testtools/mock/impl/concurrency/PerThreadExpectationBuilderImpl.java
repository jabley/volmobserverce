 ...
 */
public class PerThreadExpectationBuilderImpl
        extends ExpectationBuilderImpl
        implements PerThreadExpectationBuilder {
    ...
    public synchronized void addThreadSpecificBuilder(
        ...
        ListIterator iterator = outstandingBindings.listIterator(
                outstandingBindings.size());
        while (iterator.hasPrevious()) {
            ThreadBinder existing = (ThreadBinder) iterator.previous();
            int result = binder.matcher.compareTo(existing.matcher);
            if (result <= 0) {
                // Insert the new binder before the existing one as it either
                // has lower precedence, or has equal precedence but was added
                // after so either way it should be checked after the existing
                // one.
                iterator.set(binder);
        ...
        // The new binder has a higher precedence than any existing binder so
        // add it at the end.
    ...
    private synchronized ExpectationBuilder getThreadSpecificBuilder() {
        ...
        // See whether any of the outstanding bindings apply.
        ...
        if (builder == null) {
            throw new IllegalStateException(
                    "Could not find expectations for thread '" + thread + "'");
        }
...
//                    (ExpectationContainer) entry.getValue();
//
//            try {
//                container.verify();
//            } catch (AssertionFailedError e) {
//                AssertionFailedError augmented =
//                        new AssertionFailedError(
//                                "Thread: " + name + " - " + e.getMessage());
//                augmented.initCause(e);
//                throw e;
//            }
//        }
    }
