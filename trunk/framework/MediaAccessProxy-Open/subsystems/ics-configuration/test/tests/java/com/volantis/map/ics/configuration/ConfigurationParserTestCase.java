    ...
    public void testPort() {
        {
            String xml =
                ...
                "<environment " +
                        "imgHost='' imgPort='' " +
                         "proxyHost='' proxyPort='' " +
                        "useInMemoryIOCache='true' />" +
                "</image></transcodingServer>";

            InputStream stream = new ByteArrayInputStream(xml.getBytes());
            Configuration conf = ConfigurationParserFactory.getInstance().
                createConfigurationParser().unmarshal(stream);

            assertEquals("Host Port", 0, conf.getHostPort());
            assertEquals("Proxy Port", 0, conf.getProxyPort());
        }
        {
            String xml =
                "<?xml version='1.0'?>" +
                ...
                "<transcodingServer><image>" +
                "<environment " +
                        "imgHost='' imgPort='8000' " +
                         "proxyHost='' proxyPort='8001' " +
                        "useInMemoryIOCache='true' />" +
                "</image></transcodingServer>";
            ...
            assertEquals("Host Port", 8000, conf.getHostPort());
            assertEquals("Proxy Port", 8001, conf.getProxyPort());
        }
        {
            String xml =
                "<?xml version='1.0'?>" +
                ...
                "<transcodingServer><image>" +
                "<environment " +
                        "imgHost='' imgPort='invalid' " +
                         "proxyHost='' proxyPort='invalid' " +
                        "useInMemoryIOCache='true' />" +
                "</image></transcodingServer>";
