 ...
 */
public class URLContentManagerTestCase
    ...
    public void testETagContentChanged() throws Exception {
        ...
        // second request should contain the If-None-Match header with the ETag
        // value, the response sends a new ETag value
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
                UA_STRING,
        ...
        // third request should contain the If-None-Match header with the second
        // ETag, response send 304 Not Modified
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa2\"",
                UA_STRING,
    ...
    public void testLastModified() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                "HTTP/1.0 200 OK",
                "Date: Fri, 31 Dec 1999 23:59:59 GMT",
                "Last-Modified: Thu, 30 Dec 1999 23:59:59 GMT",
                "Content-Type: text/plain",
                "Cache-Control: public",
                "",
                "<p>hello</p>"});
        ...
        // second request with If-Modified-Since header using the Last-Modified
        // header value from the first response
        // new response indicates that the content is not modified
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-Modified-Since: Thu, 30 Dec 1999 23:59:59 GMT",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
            new String[]{
                "HTTP/1.0 304 Not Modified",
        ...
        content = manager.getURLContent(url, null, null);
        assertEquals("Content was not modified, cached content is returned",
            "<p>hello</p>\n", toString(content.getInputStream()));
        assertEquals(Cacheability.CACHEABLE,
            content.getDependency().getCacheability());
        assertEquals(Period.ZERO, content.getDependency().getTimeToLive());
    ...
    public void testLastModifiedContentChanged() throws Exception {
        ...
        // second request should contain the If-Modified-Since header with the
        // Last-Modified header value from the first response
        // second response indicates that the content was modified and new
        // Last-Modified value returned
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-Modified-Since: Thu, 30 Dec 1999 23:59:59 GMT",
                UA_STRING,
            ...
            new String[]{
                ...
                "Last-Modified: Sat, 30 Dec 2000 23:59:59 GMT",
                "Content-Type: text/plain",
                "Cache-Control: public",
                "",
                "<p>hello2</p>"});
        ...
        // third request should contain an If-Modified-Since header using the
        // Last-Modified header value from the second response
        // third response will return 304 Not Modified
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-Modified-Since: Sat, 30 Dec 2000 23:59:59 GMT",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
            new String[]{
                "HTTP/1.0 304 Not Modified",
        ...
        content = manager.getURLContent(url, null, null);
        assertEquals("Content was not modified since the second request",
            "<p>hello2</p>\n", toString(content.getInputStream()));
        assertEquals(Cacheability.CACHEABLE,
            content.getDependency().getCacheability());
        assertEquals(Period.ZERO, content.getDependency().getTimeToLive());
    ...
    public void testETagAndLastModified() throws Exception {
        ...
        // second request should contain both the If-Modified-Since and the
        // If-None-Match headers, second response is 304 Not Modified
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-Modified-Since: Thu, 30 Dec 1999 23:59:59 GMT",
                "If-None-Match: \"aa\"",
        ...
        content = manager.getURLContent(url, null, null);
        assertEquals("Content was not modified, cached value is used",
            "<p>hello</p>\n", toString(content.getInputStream()));
        assertEquals(Cacheability.CACHEABLE,
            content.getDependency().getCacheability());
        assertEquals(Period.ZERO, content.getDependency().getTimeToLive());
    ...
    public void testMaxEntryExceeded() throws Exception {
        ...
        // manager with max-entries = 1 is created
        final URLContentManager manager =
            createURLContentManager(Period.INDEFINITELY, true, 1);
        ...
        // request to check that first response was cached
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-Modified-Since: Thu, 30 Dec 1999 23:59:59 GMT",
                "If-None-Match: \"aa\"",
        ...
        // third request is for another resource, the response is cacheable
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?bar HTTP/1.1",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
            new String[]{
                "HTTP/1.0 200 OK",
        ...
        assertEquals("Content of the second resource is returned",
            "<p>world</p>\n", toString(content.getInputStream()));
        assertEquals(Cacheability.CACHEABLE,
            content.getDependency().getCacheability());
        assertEquals(Period.ZERO, content.getDependency().getTimeToLive());

        // 4th request to check if the previous response was cached
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?bar HTTP/1.1",
                "If-Modified-Since: Thu, 30 Dec 1999 23:59:59 GMT",
                "If-None-Match: \"bb\"",
        ...
        // 5th request to check that the first response is not cached (no
        // validation headers)
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
    ...
    public void testExpiresOK() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                "HTTP/1.0 200 OK",
                "Date: Fri, 31 Dec 1999 23:59:59 GMT",
                "Expires: Fri, 31 Dec 2999 23:59:59 GMT",
                "Content-Type: text/plain",
                "",
                "<p>hello</p>"});
        ...
        assertTrue(Comparator.LT.compare(
            Period.inSeconds(1000 * 365 * 24 * 60 * 60),
            content.getDependency().getTimeToLive()));

        // cached response is not expired, no new request is made
        content = manager.getURLContent(url, null, null);
    ...
    public void testExpiresOKButMaxAge() throws Exception {
        ...
        // max-age value is not big enough, so cached response is stale
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
    ...
    public void testExpiresOld() throws Exception {
        ...
        // Expires header value in the past means, cached content must be
        // revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
    ...
    public void testExpiresOldNoETag() throws Exception {
        ...
        // Expires header value was in the past, but
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
    ...
    public void testCacheControlNoCacheExpires() throws Exception {
        ...
        // even if the cache entry is not expired according to the Expires
        // header value, Cache-Conrol/no-cache means it must be revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
                UA_STRING,
    ...
    public void testCacheControlNoCacheMaxAge() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                ...
                "Date: Fri, 31 Dec 1999 23:59:59 GMT",
                "Content-Type: text/plain",
                "Cache-Control: public, no-cache, max-age=10000000",
                "ETag: \"aa\"",
                "",
                "<p>hello</p>"});
        ...
        // even if the cache entry is not expired according to the value of the
        // Cache-Control/max-age directive, Cache-Conrol/no-cache means it must
        // be revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
    ...
    public void testCacheControlNoCacheSMaxAge() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                ...
                "Expires: Fri, 31 Dec 2999 23:59:59 GMT",
                "Content-Type: text/plain",
                "Cache-Control: public, no-cache, s-maxage=100000000",
                "ETag: \"aa\"",
                "",
                "<p>hello</p>"});
        ...
        // even if the cache entry is not expired according to the value of the
        // Cache-Control/s-maxage directive, Cache-Conrol/no-cache means it must
        // be revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
    ...
    public void testPragmaNoCacheExpires() throws Exception {
        ...
        // even if the cache entry is not expired according to the Expires
        // header value, Pragma/no-cache means it must be revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
                UA_STRING,
    ...
    public void testPragmaNoCacheMaxAge() throws Exception {
        ...
        // even if the cache entry is not expired according to the value of the
        // Cache-Control/max-age directive, Pragma/no-cache means it must be
        // revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
    ...
    public void testPragmaNoCacheSMaxAge() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                ...
                "Expires: Fri, 31 Dec 2999 23:59:59 GMT",
                "Content-Type: text/plain",
                "Cache-Control: public, s-maxage=100000000",
                "Pragma: no-cache",
                "ETag: \"aa\"",
                "",
                "<p>hello</p>"});
        ...
        // even if the cache entry is not expired according to the value of the
        // Cache-Control/s-maxage directive, Pragma/no-cache means it must be
        // revalidated
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                "If-None-Match: \"aa\"",
    ...
    public void testCacheControlPrivate() throws Exception {
        ...
        // first response contained Cache-Control/private so new request is
        // expected without validation headers.
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
    ...
    public void testCacheControlMaxAge() throws Exception {
        ...
        assertEquals(Freshness.FRESH,
            content.getDependency().freshness(null));
        assertTrue(Comparator.GE.compare(Period.inSeconds(50),
            content.getDependency().getTimeToLive()));
        assertTrue(Comparator.LT.compare(Period.inSeconds(45),
            content.getDependency().getTimeToLive()));

        // The entry in the cache is still fresh, no need to revalidate.
        content = manager.getURLContent(url, null, null);
    ...
    public void testCacheControlSMaxAge() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                ...
                "Date: " + RFC1123.format(new Date()),
                "Content-Type: text/plain",
                "Cache-Control: s-maxage=50, max-age=0",
                "ETag: \"aa\"",
                "",
                "<p>hello</p>"});
        ...
        // Cache-Control/s-maxage returned in the first response means the entry
    ...
    public void testCacheControlSMaxAgeExpired() throws Exception {
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                ...
                "Date: " + RFC1123.format(new Date()),
                "Content-Type: text/plain",
                "Cache-Control: s-maxage=10, max-age=50",
                "ETag: \"aa\"",
                "",
                "<p>hello</p>"});
    ...
    public void testAge() throws Exception {
        ...
        // Age header value in the previous response is higher than the value of
        // the Cache-Control/max-age directive, so the cache entry immediately
        // become stale, revalidation is needed
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
    ...
    public void testVary() throws Exception {
        ...
        // The previous response contained the Vary header, so no caching should
        // happen: new request is needed without validation headers to reget the
        // content
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
    ...
    public void testHopByHopResponseHeaders304Uncacheable() throws Exception {
        // First request is cacheable so no hop-by-hop header should be present
        // in the returned HTTP content
        String currentDate = RFC1123.format(new Date());
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
    ...
    public void testWarningHeaders() throws Exception {
        // response with both 1xx and 2xx Warning headers
        serverMock.addTransaction(new String[]{
                "GET /blah.txt?foo HTTP/1.1",
                UA_STRING,
                "Host: " + serverMock.getServerAddress()},
    ...
    public void testResponseCookies() throws Exception {
        ...
        serverMock.addTransaction(new String[]{
            ...
            new String[]{
                ...
                "Content-Type: text/plain",
                "Cache-Control: public",
                "Set-Cookie: hello=world; domain=www.example.com; " +
                    "path=/hello/world; comment=\"hello world\"; max-age=100; " +
                    "version=1",
    ...
    public void testDependencyStored() throws Exception {
        ...
        assertTrue(Comparator.LT.compare(Period.inSeconds(5),
            content.getDependency().getTimeToLive()));
        // check if content has been removed from the dependency context
    ...
    public static String toString(final InputStream is) throws IOException {
        ...
        for (int len = reader.read(buffer); len != -1;
    ...
    public void readPauseRead(InputStream is, int pauseInMillis) throws Exception {
        try {
            ...
            // Pause this thread so that the
            final long startTime = System.currentTimeMillis();
            while (System.currentTimeMillis() - startTime < pauseInMillis) {
                // do nothing
                Thread.yield();
            }
            // try reading again the rest of the input stream
            for (int count = reader.read(buffer); count != -1;
     ...
     */
    private URLContentManager createURLContentManager(
            final Period timeout, final boolean cacheable, final int maxCount) {
        return createURLContentManager(
            SystemClock.getDefaultInstance(), timeout, cacheable, maxCount);
    }
    ...
    private URLContentManager createURLContentManager(final SystemClock clock,
        ...
        final URLContentManagerFactory factory =
     ...
     * machines.</p>
     */
    public void notestConnectFailures()
            throws Exception {
        ...
        for (int i = 0; i < 2000; i += 1) {
            try {
                URLContent content = manager.getURLContent(
                        "http://127.0.0.1:" + port, null, null);
                ...
                assertEquals("Connection refused when connecting to " +
                        "/127.0.0.1:" + port,
                        expected.getMessage());
            }
        }
    ...
    public void testInterruptingDuringReadFromFile() throws Exception {
        ...
        try {
            ...
            // Read the contents of this input stream.
    ...
    private URLContent readFromFile(String filename, Period timeout)
            throws IOException {

        final URLContentManager manager = createDefaultURLContentManager();
        URL fileToRead =
                URLContentManagerTestCase.class.getResource(filename);
