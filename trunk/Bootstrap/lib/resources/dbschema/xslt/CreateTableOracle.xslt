...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:sql="http://www.volantis.com/xmlns/internal/sql">
...
CREATE TABLE </xsl:text>
    ...
    <xsl:template match="sql:field" mode="fields">
        ...
        <!--  -->    
        <!-- don't include the field size if it's a DATE -->
        <xsl:choose>
            <xsl:when test="(sql:datatype/text() = 'DATE')">
                <xsl:choose>
                    <xsl:when test="($version='true')">
            ...
            <xsl:when test="(sql:datatype/text() = 'TIMESTAMP')">
                <xsl:choose>
                    <xsl:when test="($version='true')">
                        <xsl:text>TIMESTAMP </xsl:text>
                    </xsl:when>
            ...
            <xsl:when test="(sql:datatype/text() = 'BOOLEAN')">
                <xsl:text>NUMERIC(1) </xsl:text>
            </xsl:when>
            <xsl:when test="sql:datatype = 'VARCHAR' and sql:size > 4000">
        ...
        <xsl:if test="sql:default">
            ...
            <xsl:choose>
                ...
                <xsl:when test="sql:datatype='BOOLEAN' and sql:default='true'">
                    <xsl:text>1</xsl:text>
                </xsl:when>
                <xsl:when test="sql:datatype='BOOLEAN' and sql:default='false'">
                    <xsl:text>0</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="sql:default"/>
...
ALTER TABLE </xsl:text>
            ...
            <xsl:choose>
                <xsl:when test="string-length(sql:name) &gt; 27">
                    <xsl:value-of select="substring(sql:name,1,27)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="sql:name"/>
                </xsl:otherwise>
...
<!--
 ...
 20-Jan-05	6688/1	ianw	VBM:2005010606 Minor DB fix
