 ...
 */
public class GetPolicyValueFunction extends DeviceAccessorFunction {
    ...
    public Value invoke(ExpressionContext expressionContext,
        ...
        // only expect a single argument that will name a device policy
        assertArgumentCount(values, 1);

        // retrieve the named policy
        String policyName = values[0].stringValue().asJavaString();
