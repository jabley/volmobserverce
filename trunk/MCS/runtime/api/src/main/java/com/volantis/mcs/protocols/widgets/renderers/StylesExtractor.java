...
public final class StylesExtractor {
    ...
    private static final LogDispatcher logger = LocalizationFactory
            .createLogger(StylesExtractor.class);
     ...
     */
    private static final StyleKeyword DEFAULT_MCS_EFFECT_STYLE =
        (StyleKeyword)StylePropertyDetails.MCS_EFFECT_STYLE.getStandardDetails().getInitialValue();
    
    
    /**
     * default value of mcs-effect-duration style property.
     */
    private static final StyleTime DEFAULT_MCS_EFFECT_DURATION =
        (StyleTime)StylePropertyDetails.MCS_EFFECT_DURATION.getStandardDetails().getInitialValue();
    

    /**
     * default value of mcs-frame-rate style property.
     */
    private static final StyleFrequency DEFAULT_MCS_FRAME_RATE =
        (StyleFrequency)StylePropertyDetails.MCS_FRAME_RATE.getStandardDetails().getInitialValue();
     ...
     */
    protected StylesExtractor(Styles styles, StatefulPseudoClass pseudoClass) {
        this.styles = styles;
    ...
    private StyleValue getStyleValue(StyleProperty property) {
        ...
        if (styles != null) {
            // If pseudo-class is provided, read style value from pseudo-class
            // first.
            if (pseudoClass != null) {
                // Retrieve styles for pseudo-class.
                Styles nestedStyles = styles.findNestedStyles(pseudoClass);

                // If there are styles for the pseudo-class...
                if (nestedStyles != null) {
                    // ...retrieve style value out of it.
                    value = nestedStyles.getPropertyValues().getComputedValue(
                            property);
                }
            }

            // If no value could be found in pseudo-class,
            // try with base styles.
            if (value == null) {
    ...
    public List getEffectRules() {
        ...
        int listSize = effectsDescList.size() < durationList.size() ?
                effectsDescList.size() : durationList.size();
        // list size is min from all list sizes
        listSize = listSize < frameList.size() ? 
                listSize : frameList.size();
        if(!((effectsDescList.size() == durationList.size()) && 
                (effectsDescList.size() == frameList.size()))){
            logger.warn("repeater-parameters-list-not-equals", new Object []{
                new Integer(effectsDescList.size()),
                new Integer(durationList.size()),
                new Integer(frameList.size()) });
        }
        // fill list because they should be not shorter than effectDescList
        durationList = fillListWith(durationList,effectsDescList.size(),
                DEFAULT_MCS_EFFECT_DURATION);
        frameList =  fillListWith(frameList,effectsDescList.size(),
                DEFAULT_MCS_FRAME_RATE);
    ...
    public double getEffectDuration() {
        ...
        // This call guarantees that the list is not null and not empty 
        List durationList = getEffectDurationList();       
    ...
    public double getFrameRate() {
        ...
        // This call guarantees that the list in not null and not empty 
    ...
    public double getDelay() {
        return getTime(StylePropertyDetails.MCS_DELAY, TimeUnit.S,
                1.0);
    }
    ...
    public double getTransitionInterval() {
        return getTime(StylePropertyDetails.MCS_TRANSITION_INTERVAL,
                TimeUnit.S, 1.0);
    }
    ...
    private String getEffectStyle(StyleValue effectValue) {
        ...
        if (effectValue instanceof StyleString) {
            ...
            logger.warn("unexpected-mcs-effect-style-type",
                    effectValue.getStyleValueType().getType());
    ...
    private double getTime(StyleProperty property, TimeUnit unit,
            double defaultTime) {
