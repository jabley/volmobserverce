 ...
 * and returns the width component (i.e. if aspect ratio = width x height,
 * then this returns width). If the repository contains no entry for aspect
 * ratio then the supplied default value will be returned. If this default
 * value is not valid, zero will be returned.
 */
public class DIAspectRatioWidthFunction extends DIAspectRatioFunction {
    ...
    protected Value execute(ExpressionContext expressionContext,
        ...
        return calculateValue(expressionContext, accessor, defaultValue,
                AspectRatioOutputFormat.WIDTH);
    }

    // Javadoc inherited.
    protected String getFunctionName() {
        return "cssmq-device-aspect-ratio-width";
    }
}
