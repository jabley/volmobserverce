...
*/
/* ----------------------------------------------------------------------------
 * $Header: /src/voyager/testsuite/unit/com/volantis/mcs/protocols/html/xhtmlfull/XHTMLFullTransTableTestCase.java,v 1.2 2003/01/15 12:42:10 philws Exp $
 * ----------------------------------------------------------------------------
...
/**
 * Tests the XHTMLFullTransTable. Note that this does not include testing
 * the usage of the TransTable, but merely the specialist behaviour.
 *
 * @author <a href="mailto:phil.weighill-smith@volantis.com">Phil W-S</a>
 */
public class XHTMLFullTransTableTestCase extends TransTableTestAbstract {
    ...
    protected String[] getExpectedPreserveStyleAttributes() {
        return new String[] {
            "abbr", "align", "axis", "char", "charoff", DIR, "headers",
    ...
    protected void setInnerTableCanOptimizeExpectations(HashMap expectedValues) {
        ...
        innerTable.expects.getAttributeValue(SUMMARY).
                returns((String) expectedValues.get(SUMMARY)).min(0).max(1);
...
//                returns(false).any();
    ...
    protected void setOuterTableCanOptimizeExpectations(HashMap expectedValues) {
        ...
        outerTable.expects.getAttributeValue(TITLE).
                returns((String) expectedValues.get(TITLE)).min(0).max(1);
...
//                returns(false).any();
    ...
    public void testCannotOptimizeStyleIfKeyAttributesNonNull() {
        ...
        for (int i = 0; i < EVENT_ATTS.length; i++) {
            checkCanOptimizeStyleIfKeyAttributesNonNull(EVENT_ATTS[i], INNER);
            checkCanOptimizeStyleIfKeyAttributesNonNull(EVENT_ATTS[i], OUTER);
        }
