...
import com.volantis.mcs.runtime.ScriptLibraryManagerMock;
...
public class HighlightTransformerTestCase extends TestCaseAbstract {
     ...
     * focusable element
     */
    public void testHighlighNavigationForFocusable() throws  Exception {
    ...
    protected DOMProtocol buildExpectations(DOMProtocol protocol) {
        ...
        ScriptLibraryManagerMock slmMock = new ScriptLibraryManagerMock(
                "slm", expectations, protocol);
        pageCtxMock
            .expects.getEnvironmentContext()
            .returns(envCtxMock);
        envCtxMock
            .fuzzy.getRealPath(mockFactory.expectsInstanceOf(String.class))
            .returns("");
        pageCtxMock
            .expects.getVolantisBean()
            .returns(volantisMock);
        volantisMock
            .expects.getProjectManager()
            .returns(projMgrMock);
        projMgrMock
            .fuzzy.getProject(mockFactory.expectsInstanceOf(String.class), null)
            .returns(projectMock);
        pageCtxMock
            .expects.getPolicyReferenceFactory()
            .returns(policyRefFactoryMock).any();
        policyRefFactoryMock
            .fuzzy.createNormalizedReference(
                 projectMock, mockFactory.expectsInstanceOf(String.class), PolicyType.SCRIPT)
            .returns(null).any();
        pageCtxMock
            .expects.getAssetResolver()
            .returns(null).any();
        pageCtxMock
            .expects.getScriptLibraryManager()
            .returns(slmMock).any();
        slmMock
            .fuzzy.addScript(mockFactory.expectsInstanceOf(ScriptAssetReference.class))
            .returns(true).fixed(scriptsCount);
        pageCtxMock
