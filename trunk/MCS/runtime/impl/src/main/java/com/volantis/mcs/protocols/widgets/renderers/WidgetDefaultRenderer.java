...
public abstract class WidgetDefaultRenderer implements WidgetRenderer {
    ...
    /**
     * The method should be called to indicate that given JS library is required
     *
     * @throws ProtocolException
     *
     */
    protected void requireLibrary(String libraryName,VolantisProtocol protocol)
            throws ProtocolException {
        WidgetHelper.requireLibrary(libraryName, protocol);

        // The 'vfc-base.js' library when loaded, requires the Widget.startup()
        // to be called.
        if (libraryName.equals("/vfc-base.mscr")) {
            getWidgetDefaultModule(protocol).requireStartup();
        }
    }
    ...
    /**
     * Add requirements for standard libraries
     *
     * @throws ProtocolException
     */
    protected void requireStandardLibraries(VolantisProtocol protocol)
            throws ProtocolException {

        requireLibrary("/prototype.mscr", protocol);
        requireLibrary("/scriptaculous.mscr", protocol);
        requireLibrary("/vfc-base.mscr", protocol);
        requireLibrary("/vfc-custom-effects.mscr",protocol);
    }

    /**
     * Retrieve marker value when folding-item folded
     * @param protocol
     * @param attributes
     * @return
     */
    protected StyleValue getFoldedMarkerValue(VolantisProtocol protocol, MCSAttributes attributes){
        Styles styles = attributes.getStyles();
        Styles markerStyles = styles.findNestedStyles(PseudoElements.MARKER);
        StyleValue markerValue = null;
        if (markerStyles != null) {
            markerValue = markerStyles.getPropertyValues().getComputedValue(
     ...
     * @param protocol
     * @param markerValue
     * @param alternativeTextValue
     * @return
     */
    protected String renderFoldedMarker(VolantisProtocol protocol,StyleValue markerValue,
    		String alternativeTextValue) throws ProtocolException{
        DOMOutputBuffer currentBuffer = getCurrentBuffer(protocol);
        Element markerElement = currentBuffer.addElement("span");

        markerElement.setStyles(StylingFactory.getDefaultInstance().createInheritedStyles(
        ...
        } else if(null != alternativeTextValue){
        	markerElement.addText(alternativeTextValue);
        } else {
     ...
     * @throws ProtocolException
     */
    protected String renderUnfoldedMarker(VolantisProtocol protocol,StyleValue markerValue,
    		String alternativeTextValue) throws ProtocolException {
        DOMOutputBuffer currentBuffer = getCurrentBuffer(protocol);
        Element markerElement = currentBuffer.addElement("span");
    ...
    /**
     * Retrieve marker value when folding-item unfolded
     * @param protocol
     * @param attributes
     * @return
     */
    protected StyleValue getUnfoldedMarkerValue(VolantisProtocol protocol, MCSAttributes attributes){
        Styles styles = attributes.getStyles();
        Styles unfoldedStyles = styles.findNestedStyles(StatefulPseudoClasses.MCS_UNFOLDED);
        StyleValue markerValue = null;
        if(null != unfoldedStyles){
	        Styles markerStyles = unfoldedStyles.findNestedStyles(PseudoElements.MARKER);
