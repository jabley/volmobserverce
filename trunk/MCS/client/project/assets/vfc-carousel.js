...
//end user parameters
    ...
    setCarouselStyle: function(element){
      element.setStyle({'height' : 'auto', 'width' : this.carWidth});
    },
    ...
    fillSwapBlockContent: function(view,reverse) {
      ...
      this.effectElement.setStyle({'visibility' : 'hidden', 'display' : 'block'});
      while(!overflow)
      {
 ...
 */
  changePageInternal: function(finalState, nextPage) {
    ...
    if (this.swapable.effectState != 'idle') return;
...
Widget.Carousel = Class.define(Widget.Swapable,Widget.Refreshable,Widget.Focusable ,{
  ...
  getPaddingAndBorder: function(element,propertiesArray){
    ...
    for(i = 0;i<propertiesArray.length;i++){
      var v = parseInt(this.element.getStyle(propertiesArray[i]));
      if(v > 0){
        result+=v;
      }
    }  
  ...
  fixCarouselDimensions: function(){
    if(!this.initialSize){
      var carouselDimension = this.element.getDimensions();
  ...
  setContentElementsInternal: function(elements) {
    ...
    //only element in page DOM have proper size values
    this.element.appendChild(tmpStorage);
  ...
  setContentElementsNow: function(elements) {
    // TODO: Don't know how to do it yet, so by now use setContent()
    this.setContentElements(elements)
  },
