...
Widget.log("DatePicker","calendar setDisabledWeekDays");
  ...
  */
  createTableBody : function(){
    ...
    for (var row=1; row<=6; row++) {
        ...
        for (var col=1; col<=7; col++) {
          ...
          td = Element.extend(td)
          this.tds[i] = td;
          this.divs[i] = Element.extend(document.createElement('DIV'))
          Element.setStyle(this.divs[i],{'text-align':'center'});
  ...
  netfrontButtonSetInnerHTML : function(el,v){
      el.innerHTML = "<input type='button' style='width:1px; height:1px; backgroundColor:transparent; border:0px'>"+v;
  },
	  ...
	  var i = 1;
          ...
          for (var row=1; row<=6; row++) {
              for (var col=1; col<=7; col++) {    
                  ...
                  if(Prototype.netFront()) this.netfrontButtonSetInnerHTML(this.divs[i],this.display_date)
...
//types selected date to input field
  returnDate : function(day){  
    ...
    if  (day>-1){
      ...
      if (Prototype.netFront()) i = this.netfrontButtonGetInnerHTML(this.divs[day]);
