 ...
 * cache mechanism.
 */
public class CachingRequestExecutor implements
        HTTPRequestExecutor {
    ...
    private static Category logger = Category.getInstance(
            "com.volantis.xml.pipeline.sax.drivers.web.httpcache" +
            "CachingRequestExecutor");
     ...
     */
    public CachingRequestExecutor(final Cache cache,
                                  final AbstractPluggableHTTPManager target,
                                  final String url,
                                  final HTTPRequestType requestType,
                                  final HTTPVersion version,
                                  final ProxyManager proxy,
                                  final XMLPipelineContext xmlPipelineContext) {
     ...
     */
    public HTTPResponseAccessor execute()
            throws HTTPException {

        // all the request parameters have now been added to the executor so
        // we can rebuild the url with its query parameters to create the
        // cache key string.
        final String url = HTTPManagerUtilities.createQueryURL(
     ...
     * error.
     */
    void transferMetaData(HTTPRequestExecutor executor)
            throws HTTPException {

        // copy the parameters references
    ...
    public void addRequestParameter(RequestParameter parameter)
        ...
        if (parameter != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("null passed as parameter to " +
                        "addRequstParameter: ignoring it");
            }
        }
    }
    ...
    public void addRequestHeader(Header header) throws HTTPException {
        if (header != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("null passed as parameter to addRequstHeader" +
                        ": ignoring it");
            }
        }
    }
    ...
    public void addRequestCookie(Cookie cookie) throws HTTPException {
        if (cookie != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("null passed as parameter to addCookie" +
                        ": ignoring it");
            }
        }
    }
    ...
    private boolean canRequestBeSatisfiedByCache(HTTPMessageEntities headers) {
        boolean result = true;
        if (headers != null) {
            // check for "Cache-Contol: no-cache"

            result = !findHeaderMatch(headers,
                HTTPCacheHeaders.CACHE_CONTROL.getIdentity(),
                HTTPCacheHeaders.NO_CACHE);
            ...
            if (result) {
                result = !findHeaderMatch(headers,
                    HTTPCacheHeaders.PRAGMA.getIdentity(),
                    HTTPCacheHeaders.NO_CACHE);
                if (!result && logger.isDebugEnabled()) {
     ...
     * </code>.
     */
    private boolean findHeaderMatch(HTTPMessageEntities headers,
                                    HTTPMessageEntityIdentity header,
                                    String match) {
