            ...
            if (requestParameters != null && requestParameters.size() > 0) {
                ...
                if (idx == -1 || forceAppend) {
                    ...
                    while (it.hasNext()) {
                        ...
                        //
                        // NOTE: here we render parameters without a value
                        // without the "=" separator regardless of whether it 
                        // was present in the original URL. For example: 
                        //
                        // input: "a=" => output: "a"
                        // input: "a"  => output: "a"
                        //
                        // Previously we always rendered the "=" for valueless
                        // parameters which broke a Turkcell DSB service so it
                        // was changed. Neither way is good as really we 
                        // should reconstruct the output URL that DSB uses 
                        // exactly as it came in. Hopefully, however, the "a" 
                        // form will be less likely to break applications than
                        // the "a=" form.
                        // 
                        // We are unable to render it properly because the 
                        // caller apparently passes "" as value for both cases 
                        // (which is probably due to the similar limitiation 
                        // on request.getParameterXxx).
                        //
                        // If we discover an application which requires 
                        // valueless parameters of the "a=" form then we shall
                        // need to come up with a general case solution to 
                        // this problem, for example, recording the input
                        // exactly, including the use of "=" for valueless
                        // parameters (and hopefully parameter position).
                        //
                        // See VBM:2006081007 for more details.
                        //
    ...
    private static HTTPMessageEntity[] aquireOutgoingHTTPMessageEntitys(
        ...
        if (from == null || requestEntities == null) {
            ...
            if (value != null) {
                ...
                throw new HTTPException(
                        "Entity " + entity.getName() + " has no value " +
                        "and no means to copy a value. This is not valid");
            }
        } else {
