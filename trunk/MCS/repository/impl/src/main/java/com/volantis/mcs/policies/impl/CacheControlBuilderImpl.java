...
public class CacheControlBuilderImpl
        extends AbstractBuilder
        implements InternalCacheControlBuilder {
    ...
    public CacheControlBuilderImpl(InternalCacheControl cacheControl) {
        if (cacheControl != null) {
            ...
            cacheThisPolicy = getOptionalBoolean(
                    cacheControl.cacheThisPolicyWasSet(),
                    cacheControl.getCacheThisPolicy());

            retainDuringRetry = getOptionalBoolean(
                    cacheControl.retainDuringRetryWasSet(),
                    cacheControl.getRetainDuringRetry());

            retryFailedRetrieval = getOptionalBoolean(
                    cacheControl.retryFailedRetrievalWasSet(),
                    cacheControl.getRetryFailedRetrieval());

            retryInterval = getOptionalInteger(
                    cacheControl.retryIntervalWasSet(),
                    cacheControl.getRetryInterval());

            retryMaxCount = getOptionalInteger(
                    cacheControl.retryMaxCountWasSet(),
                    cacheControl.getRetryMaxCount());

            timeToLive  = getOptionalInteger(
                    cacheControl.timeToLiveWasSet(),
    ...
    protected boolean castThenCheckEquality(Object obj) {
        return (obj instanceof CacheControlBuilderImpl) ?
                equalsSpecific((CacheControlBuilderImpl) obj) : false;
    }
