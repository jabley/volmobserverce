...
<xsl:stylesheet xmlns:common="http://exslt.org/common" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0" exclude-result-prefixes="common xsi">
 ...
 ! ============================================================================
 ! This document defines the schema for the whole XDIME CP Language.
 ! ======================================================================== </xsl:comment>
    </xsl:template>
    ...
    <xsl:template match="xs:import[@schemaLocation = 'diselect.xsd']">
        <xsl:call-template name="ImportSchema"/>
    </xsl:template>

    <xsl:template match="xs:import[@schemaLocation = 'widgets.xsd']">
        <xsl:call-template name="ImportSchema"/>
    </xsl:template>

    <xsl:template match="xs:import[@schemaLocation = 'widgets-response.xsd']">
        <xsl:call-template name="ImportSchema"/>
    </xsl:template>
    ...
    <xsl:template match="xs:import[@schemaLocation = '../xml-events.xsd']">
        <xsl:call-template name="ImportSchema"/>
    </xsl:template>
