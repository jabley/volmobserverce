...
public class InternalMessageService implements MessageService {
     ...
     */
    public Failures process(SendRequest sendRequest, Session session)
            throws MessageServiceException {
    ...
    private void partitionRecipients(Map mapping, Recipient[] recipients) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Entered partitionRecipients with "
     ...
     */
    private MessageRecipients sendMessage(Session session,
                                          MultiChannelMessage mcMessage,
        ...
        try {
            return session.send(mcMessage,
                    RecipientType.TO.name(),
                    RecipientType.CC.name(),
                    RecipientType.BCC.name(),
                    from);
        } catch (MessageException e) {
            if (LOGGER.isDebugEnabled()) {
     ...
     *                            form to the other.
     */
    private Recipient getFailedRecipient(MessageRecipient failure)
            throws RecipientException {
        Address address = getAddress(failure);
        return MessageFactory.getDefaultInstance().createRecipient(address,
                failure.getDeviceName());
    }
     ...
     */
    private void addRecipients(Session session, Map recipients)
            throws MessageServiceException {
    ...
    private void setAddress(MessageRecipient result, final Address address) throws RecipientException {
        if (address instanceof SMTPAddress) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Unknown Address type "
