...
public class JPEGColourWriter extends DefaultWriter {
     ...
     * @throws ImageWriterException if an error occurs while writing.
     */
    private NoFlushImageOutputStream shrinkImage(RenderedOp src,
                                     ObjectParameters params,
                                     float quality,
                                     NoFlushImageOutputStream os)
        throws ImageWriterException {
        try {
            ...
            if (minQuality >= DEFAULT_QUALITY * 100f) {
                LOGGER.warn("invalid-parameter-value", new Object[] {
                    ...
                    new Integer(Math.round(DEFAULT_QUALITY * 100f))});
            ...
            while (!degradeImage(os, resizedSrc, params, quality)) {
                ...
                src = ToolFactory.getInstance().
                    getTool("ResizingTool").process(img, params)[0];
                img = null;
                if (oldImageWidth != -1) {
            ...
            String msg = EXCEPTION_LOCALIZER.format("image-writing-failure",
                                                    e.toString());
            if (LOGGER.isErrorEnabled()) {
                LOGGER.error(EXCEPTION_LOCALIZER.format("image-writing-failure",
                                                    e.toString()));
            }
            throw new ImageWriterException(e);
        }
     ...
     * @throws ImageWriterException
     */
    private boolean degradeImage(NoFlushImageOutputStream stream,
                                 RenderedOp src,
                                 Parameters params,
